...
__global__ void kernel_partial_atomics(int *data, int *results) {
	...
	if (threadIdx.x == 0) {
		for (i = 0; i < BLOCK_SIZE; i++) {
			atomicAdd(&results[INDEX_SUM], data[i + blockIdx.x * BLOCK_SIZE]);
			atomicMax(&results[INDEX_MAX], data[i + blockIdx.x * BLOCK_SIZE]);
			atomicMin(&results[INDEX_MIN], data[i + blockIdx.x * BLOCK_SIZE]);
		}
	}	
}
